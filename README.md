...
### 阶段 1：架构重构 ✅
- 三层架构（core/services/cli）
- 全面类型注解
- 单元测试覆盖

### 阶段 2：体验优化 ✅
- **掌握度追踪**：`--confidence` 参数动态调整复习间隔
- **图表可视化**：饼图、柱状图、热力图嵌入 PDF 报告
- **难度分级**：`--difficulty` 参数筛选题目难度（1-5 星）
- **模板扩展**：举一反三模板从 7 个扩展至 30 个知识点
- **LLM Fallback**：无模板知识点自动调用 LLM 生成
