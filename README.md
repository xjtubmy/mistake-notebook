...
[![Version: 2.0.0](https://img.shields.io/badge/version-2.0.0-blue.svg)](https://github.com/silijian/mistake-notebook/releases)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)
[![Python 3.8+](https://img.shields.io/badge/python-3.8+-blue.svg)](https://www.python.org/downloads/)
[![Coverage: 89%](https://img.shields.io/badge/coverage-89%25-green.svg)](https://github.com/silijian/mistake-notebook)
...
| 方向 | 能力 |
|------|------|
| 录入与归档 | 从照片或文字整理错题，补齐 `type: mistake-record` 元数据（学科、知识点、错因等），落到 `students/{学生}/mistakes/`；生成格式适配 Obsidian **Properties** 与 **`[[双向链接]]`** |
...
| 复习进度 | 说「复习完了」「物理复习完了」等，自动批量更新**今日已到期**的题目；**支持 `--confidence` 设置掌握度**（low/medium/high），动态调整下次复习间隔 |
| 巩固与诊断 | 举一反三 / 变式题（**支持 `--difficulty` 难度筛选**）、薄弱知识点、综合分析、月度报告 |
| 📊 **图表可视化** | 自动生成 **饼图**（学科分布）、**柱状图**（错误类型）、**热力图**（复习密度），嵌入 PDF 报告 |
| 提醒 | 飞书、微信或 `crontab` 等渠道的每日复习提醒（可 dry-run 预览文案） |
| 🆕 **知识库模式** | 基于卡帕西 LLM Wiki 理念，把错题本升级成结构化知识库（知识点图谱、双向链接、自动 Lint） |
| 🆕 **学生档案初始化** | 交互式创建学生档案（`init-student.py`），自动建立目录结构和教材映射 |
| 🆕 **E2E 测试** | 16 个端到端测试覆盖核心工作流，确保功能稳定性 |
...
`generate-practice.py` 已扩展至 **30 个知识点**，覆盖物理、数学、英语、化学：
...
```bash
# 按难度范围筛选（1 最简单，5 最难）
python3 scripts/generate-practice.py --student "张三" --knowledge "欧姆定律" --difficulty 1-3

# 按风格筛选（自动映射到难度）
python3 scripts/generate-practice.py --student "张三" --knowledge "二次函数" --style 基础
python3 scripts/generate-practice.py --student "张三" --knowledge "二次函数" --style 提升
```
...
- **模板扩展**：举一反三模板从 7 个扩展至 30 个知识点（物理 10、数学 8、英语 7、化学 5）
- **PDF 美化**：改进 CSS 模板，支持中文排版

### 阶段 3：巩固完善 ✅
- **完整文档**：CONTRIBUTING.md、docs/ 目录（6 篇详细文档）
- **E2E 测试**：16 个端到端测试覆盖核心工作流
- **性能优化**：脚本启动速度提升 40%
- **学生档案初始化**：`init-student.py` 交互式创建学生档案
- **开发者贡献指南**：代码规范、测试流程、发布流程
