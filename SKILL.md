...
2. 默认导出 PDF
3. 未指定 `--output` 时，脚本写入**固定约定路径** `data/mistake-notebook/students/<学生>/exports/最新-全部.pdf` 或 `最新-<学科>.pdf`（中文文件名，每次覆盖同名文件），便于飞书/cron 用常量路径上传；标准输出含一行 `OUTPUT_PATH=<绝对路径>` 可供下游解析
